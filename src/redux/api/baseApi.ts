...
const baseQuery = fetchBaseQuery({
  baseUrl: "https://gardening-project-backend.vercel.app/api/",
  credentials: "include",
...
> = async (args, api, extraOptions): Promise<any> => {
  ...
  if (result?.error?.status === 401 || result?.error?.status === 500) {
    ...
    const res = await fetch("https://gardening-project-backend.vercel.app/api/auth/refresh-token", {
      method: "POST",
      credentials: "include",
    });
