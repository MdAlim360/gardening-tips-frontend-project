...
export async function middleware(request: NextRequest) {
  ...
  //     return NextResponse.next();
  // }
  if (role === "user" && pathname === "/people") {
    return NextResponse.next();
  }
  if (role === "user" && pathname === "/gallery") {
    return NextResponse.next();
  }
  if (role === "user" && pathname === "/premium") {
    return NextResponse.next();
  }
  if (role === "user" && pathname === "/about") {
    return NextResponse.next();
  }
  if (role === "user" && pathname === "/contact") {
    return NextResponse.next();
  }
  ...
  if (role === "user") {
    // Allow user access to dashboard, profile, and other user-specific pages
    if (
